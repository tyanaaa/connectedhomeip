...
EmberAfStatus OnOffServer::setOnOffValue(chip::EndpointId endpoint, chip::CommandId command, bool initiatedByLevelChange)
{
	...
	if (command == Commands::OnAudio::Id || command == Commands::Toggle::Id ) {
		...
		system("gst-launch-1.0 filesrc location=/data/matter.mp3 ! mpegaudioparse ! mpg123audiodec ! pulsesink volume=0.5");
...
bool emberAfOnOffClusterOnAudioCallback(app::CommandHandler * commandObj, const app::ConcreteCommandPath & commandPath,const Commands::OnAudio::DecodableType & commandData)
{
